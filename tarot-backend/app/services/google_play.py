...
class GooglePlayService:
    ...
    async def verify_purchase(
        ...
        try:
            ...
            user = await user_service.get_or_create_user(db, purchase_request.installation_id)
            ...
            purchase = Purchase(
                ...
                user_id=user.id,
                product_id=int(purchase_request.product_id.split('.')[-1]),  # Extract product ID
            ...
            new_balance = await user_service.update_user_balance(
                db=db,
                user_id=user.id,
                credit_change=purchase.credits,
            ...
            logger.info(f"Successfully processed Google Play purchase: {purchase.order_id}")

            return GooglePlayPurchaseResponse(
                success=True,
                message="Purchase verified and credits awarded",
                order_id=purchase.order_id,
                credits_awarded=purchase.credits,
