...
- category: 该维度所属的大类（例如 情感、事业、健康、人际、学业、决策 等）。
- description: 对该维度的文字描述，用1、2句话来描述，解读的关键信息。（中文或多语言文本）。
- aspect: 与三牌阵（过去/现在/将来）直接关联时，取固定值之一："过去"、"现在"、"将来"。对于非三牌阵的子维度（例如互动、角色、状态等），aspect 可为描述性文本，解读的关键信息。
